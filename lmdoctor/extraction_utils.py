...
class Extractor:
    def __init__(self, model, tokenizer, user_tag, assistant_tag, device='cuda:0', 
                 extraction_target=None, extraction_method=None, direction_op='pca',
                 **kwargs):
        """
        ...
        self.direction_op = direction_op
        self.device = device
        self.kwargs = kwargs
    ...
    def extract(self, batch_size=8, n_train_pairs=128, n_dev_pairs=64, n_test_pairs=32):
        ...
        self.direction_info = get_directions(self.train_acts, self.device, self.direction_op)
...
def get_directions(train_acts, device, method):
    if method == 'pca':
...
def get_directions_logreg(train_acts, device):
    directions = {}
    scaled_directions = {}
    direction_info = defaultdict(dict)
    for layer in train_acts:
        ...
        labels = [1, 0] * n_pairs
        clf = train_logreg(acts.cpu().numpy(), np.array(labels))
...
def train_logreg(X, y, test_clf=False):
